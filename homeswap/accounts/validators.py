    ...
    if country_code == 'Any country':
        for regex in POSTAL_CODE_REGEXES.values():
            if re.match(regex, value):
                return True
        raise ValidationError({'postal_code': f'Invalid postal code format: {value}'})
    
    regex = POSTAL_CODE_REGEXES.get(country_code)
    
    if regex is None:
        raise ValidationError({'postal_code': f'Invalid country code: {country_code}'})
    
    if not re.match(regex, value):
        raise ValidationError({'postal_code': f'Invalid postal code format for {country_code}: {value}'})
